...
Page({
  ...
  onLoad: function (options) {
    ...
    wx.setStorageSync('entrance', 'hiszlx');
    app.editTabbar();
  ...
  toNextpage(e) {
    ...
    if (!wx.getStorageSync('userPhone')) { //判断是否包含userPhone字段
      ...
      if(topage=="zwjs"){
        wx.navigateTo({
          url: '/pages/zwpp/ahszsz/zwpp',
        })
      }else if(topage=="lnfs"){
        wx.navigateTo({
          url: '/pages/lnfs/ahszsz/lnfs',
        })
      }else if(topage=="bmrs"){
        wx.navigateTo({
          url: '/pages/bmrs/bmrs?fxname='+wx.getStorageSync('fxname')+'&actid=31455',
        })
      }
    }
  ...
  Register() {
    ...
    }else if(wx.getStorageSync('entrance_gn') == 'bmrs'){
      data.entrance_2 = '报名人数';
    }
    ...
    wx.request({
      ...
      success: res => {
        ...
        if (data.status == 1) {
          ...
          // 注册
          wx.request({
            url: CONFIG.RegisterDIYAPI+'31455',
            data: zc_data,
            success: res => {
              var data = JSON.parse(res.data.replace(/^(\s|\()+|(\s|\))+$/g, ''));
              console.log(data);
