...
Page({
  ...
  onLoad: function (options) {
    ...
    wx.setStorageSync('entrance', 'jsszsz');
    app.editTabbar();
  ...
  toNextpage(e) {
    ...
    if (!wx.getStorageSync('userPhone')) { //判断是否包含userPhone字段
      ...
      if(topage=="zwpp"){
        wx.navigateTo({
          url: '/pages/zwpp/jsszsz/zwpp',
        })
      }else if(topage=="lnfs"){

        wx.navigateTo({
          url: '/pages/lnfs/jsszsz/lnfs',
        })
      }
    }
  ...
  Register() {
    ...
    data.city_or_school = this.data.fxname;
    data.entrance_1 = '江苏省直职位检索';
    ...
    console.log(CONFIG.RegisterAPI);
    console.log(data);
    // 注册
    wx.request({
