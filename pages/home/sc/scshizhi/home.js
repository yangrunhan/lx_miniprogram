// pages/home/sc/scshizhi/home.js
// pages/home/sc/scsz/home.js
...
Page({
  ...
  onLoad: function (options) {
    ...
    wx.setStorageSync('entrance', 'scsz');
    app.editTabbar();
    this.getIndexPageData();

    if(options.fxname){

      let tg_fxname = options.fxname
      wx.setStorageSync('tg_fxname',tg_fxname);
  ...
  Register_ScShizhi(){
    ...
    let tg_fxname = wx.getStorageSync('tg_fxname');
    if(tg_fxname){
      data.tg_fxname = tg_fxname;
    }
    ...
    // 注册
    wx.request({
      url: 'https://zg99.offcn.com/index/chaxun/register?actid=42151&callback=?',
      data: data,
      success: res => {
        var data = JSON.parse(res.data.replace(/^(\s|\()+|(\s|\))+$/g, ''));
        console.log(data);
