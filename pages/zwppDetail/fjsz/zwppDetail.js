...
Page({
  ...
  getSearchData() {
    ...
    console.log(searchdata);
    wx.request({
      url: 'https://zg99.offcn.com/index/chaxun/getlist/?actid=42236&callback=?',
      data: {
        year: searchdata.year,
        dw_type: searchdata.dwxz,
      ...
      success: res => {
        ...
        if (data.status == 1) {
          ...
          const new_array = data.lists.filter( item => {
            return (item.xueli.includes("不限")||item.xueli.includes(searchdata.xueli))&&(item.zhuanye.includes("不限")||item.zhuanye.includes(searchdata.zhuanye));
        });
